...
import { employerMessageEmail } from "@/lib/email/templates";

export async function startApplicantChat(applicationId: string) {
    const supabase = await createClient();
    const { data: { user } } = await supabase.auth.getUser();

    if (!user) return { success: false, error: "Unauthorized" };
    
    const employerProfile = await prisma.profile.findUnique({ where: { userId: user.id } });
    if (!employerProfile || employerProfile.role !== "EMPLOYER") {
    ...
    if (application.job.employerId !== employerProfile.id) {
        return { success: false, error: "You are not the employer for this application" };
    }
    ...
    try {
        await sendEmail({
            to: application.applicant.email,
            subject: `Message from ${employerProfile.fullName} regarding ${application.job.title}`,
            html: employerMessageEmail(application.applicant.fullName, employerProfile.fullName, application.job.title, conversationId)
        });
    } catch (e) {
    ...
    try {
        await createNotification(
            application.applicantId,
            "NEW_MESSAGE",
            `${employerProfile.fullName} wants to ask you questions about your application for ${application.job.title}.`,
            "New Message",
