    ...
    if (conversation.type === 'AI_SUPPORT') {
      ...
      (async () => {
        try {
          ...
          const aiResponseText = await generateAiResponse(content, context);
...
export async function getUnreadMessages(userId: string) {
  try {
    ...
    // And there are messages created AFTER the user's lastReadAt
    
    // Prisma doesn't support complex join filtering easily in one go for this specific "unread count per conversation" 
    // efficiently without raw query or careful construction.
    // Let's try a reasonable Prisma approach:
    ...
    const unreadMessages = await prisma.message.findMany({
      where: {
        ...
        createdAt: {
          gt: prisma.participant.fields.lastReadAt // This comparison isn't directly supported in standard Prisma `where` like this
        }
      } as any, // Type assertion needed because of the cross-field comparison limitation in standard types
      // Actually, standard Prisma doesn't support field comparison in `where`.
      // We have to query conversations first or use raw query.
    });

    // Let's use a raw query for performance and correctness
