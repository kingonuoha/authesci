...
export default async function ProjectPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  // Collaborator userId refers to Profile.id
  const isEmployer = project.collaborators.some(c => c.userId === profile?.id && c.role === 'OWNER');
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Completion Banner */}
      {isCompleted && (
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4 animate-in fade-in slide-in-from-top-4 duration-500">
          ...
          <div className="flex-1">
            <h3 className="text-lg font-bold text-green-800 dark:text-green-300">Project Completed</h3>
            <p className="text-green-700 dark:text-green-400 text-sm mt-1">
              This project has been successfully completed and is now in read-only mode.
            </p>
            {!isEmployer && (
              <div className="mt-3 p-3 bg-white/50 dark:bg-black/20 rounded-lg border border-green-200 dark:border-green-800/50 inline-block">
                <p className="text-sm font-semibold text-green-800 dark:text-green-300 flex items-center gap-2">
                  <Clock className="w-4 h-4" />
                  Admin has been notified for payment release. Funds are awaiting approval.
                </p>
              </div>
            )}
          </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-4">
          {(project.creator as any).companyLogoUrl && (
            <div className="relative w-16 h-16 rounded-lg overflow-hidden border border-neutral-200 dark:border-neutral-700 flex-shrink-0 bg-white dark:bg-neutral-800">
              <Image
                src={(project.creator as any).companyLogoUrl}
                alt={project.creator.institution || "Company Logo"}
                fill
        ...
        <div className="xl:col-span-4 space-y-6">
          ...
          {!isCompleted && (
            <div className="card rounded-lg border border-gray-200 dark:border-neutral-600 dark:bg-neutral-700">
              <div className="card-body p-6">
                <h6 className="text-lg font-bold mb-4 text-neutral-900 dark:text-white">Project Actions</h6>
                <ProjectActions
                  projectId={projectId}
                  isEmployer={isEmployer}
                  projectStatus={project.status}
