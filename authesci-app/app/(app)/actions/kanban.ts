...
export async function createTask(
  ...
  try {
    const { profile } = await checkAccess(projectId);
...
export async function updateTaskStatus(taskId: string, projectId: string, status: TaskStatus) {
  try {
    ...
    await logProjectActivity(projectId, profile.id, "TASK_STATUS_UPDATED", { taskId, status, title: task.title });
...
export async function deleteTask(taskId: string, projectId: string) {
  try {
    ...
    await logProjectActivity(projectId, profile.id, "TASK_DELETED", { taskId });
